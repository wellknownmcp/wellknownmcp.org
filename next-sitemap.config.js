...
// Well-known files
const wellKnownFiles = fs.readdirSync(path.resolve(__dirname, 'public/.well-known'));
  ...
  const walk = (dir) => {
    ...
    list.forEach((file) => {
      ...
      if (stat.isDirectory()) {
        ...
        const slug = fullPath.replace(baseDir, '').replace(/\\/g, '/').replace(/\.md$/, '');
...
const newsPaths = Object.entries(newsIndex).flatMap(([lang, articles]) =>
  articles.map((item) => ({
    ...
    changefreq: 'daily', // optimisé pour "news"
    priority: 0.9,
...
module.exports = {
  ...
  generateIndexSitemap: true, // ⭐️ important !
  sitemapSize: 5000,
  changefreq: 'weekly',
  priority: 0.7,
...
// OPTIONAL BONUS: HEAD-check (à utiliser dans un script postbuild si tu veux)
